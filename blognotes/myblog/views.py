...
# Importing the LoginRequiredMixin for CBV and the @login_required decorator for fun. based views
...
class CreatePostView(LoginRequiredMixin, CreateView):
    ...
    """
    # If this view(CreatePostView) is requested, without a login, it must revert to
    # the login page
    login_url = '/login/'
...
def add_comment_to_post(request, pk):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # post in Comment model foreignkey relation, attach comment to this post
            comment.post = post
            comment.save()
        ...
        form = CommentForm()
    # render out the html based on 'form' template tagging in 'comment_form.html'
